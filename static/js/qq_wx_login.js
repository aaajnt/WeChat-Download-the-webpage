    ...
    (function (pluginName) {
        ...
        $.fn[pluginName] = function (targetid,opts) {
            ...
            function qqInit(appid,goToUrl){
                ...
                // test
                // var url = 'https://graph.qq.com/oauth2.0/authorize?response_type=code&client_id=' + appid + '&redirect_uri='+encodeURIComponent('https://kftest.qq.com/cgi-bin/qqConnectLogin');
            ...
            function handleQQLogin(event){
                ...
                var origin = e.origin
                if(data=="qqConnect"){
                    if(origin == 'https://kf.qq.com' || origin == 'https://kftest.qq.com'){
                        location.reload()
            ...
            function typeSelectHtml(){
                ...
                html += '<div class="m-login m-select-login-box" style="display:none">';
                    html += '<i class="btn_close login-font icon-login__close"></i>'
                    html += '<div class="login-tab">';
                        html += '<div class="tab-item">选择登录账号类型</div>';
                    html += '</div>';
                    html += '<div class="login-style">';
                        html += '<div class="login-item">请以使用产品时遇到问题的账号进行登录。</div>';
                        html += '<div class="login-fail">若账号无法登录，可<a data-kftag="login.cannot_login" target="_blank" href="https://kf.qq.com/product/zhzh.html">点击这里</a></div>'
